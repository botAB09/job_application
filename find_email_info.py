...
def find_email_info(driver):
    mailto_url=driver.find_element(by=By.XPATH, value='//a[@class="recruiter-email-link s-link"]').get_attribute('href').replace('%20', ' ')
    ...
    location=driver.find_element(by=By.XPATH, value='//div[@class="location"]//following-sibling::span//following-sibling::span').text
    try:location=location[:location.index('•')]
    except: pass
    print(location)
